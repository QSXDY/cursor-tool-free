    ...
    def is_cursor_running(self) -> bool:
        ...
        try:
            ...
            if self.platform == 'darwin':
                ...
                is_running = any('/Applications/Cursor.app/Contents/MacOS/Cursor' in proc 
                ...
                result = subprocess.run(['ps', '-A'], capture_output=True, text=True)
                processes = result.stdout.splitlines()
                is_running = any('cursor' in proc.lower() and 
                               'cursorservice' not in proc.lower() and 
                               (proc.lower().strip().endswith(' cursor') or 
                                'cursor.app' in proc.lower())
                               for proc in processes)
                self.logger.info(f'Cursor主进程是否运行: {is_running}')
                return is_running
