...
class UsageUpdateThread(QThread):
        ...
        try:
            ...
            if basic_info['subscription'] in ['pro', 'free_trial']:
                ...
                if basic_info['subscription'] == 'pro':
                    ...
                    print(f"🔍 [Session Token] 格式: user_id%3A%3A[JWT_token]")
    ...
    def _get_aggregated_usage_cost(self, session_token):
        ...
        try:
            ...
            print(f"🔍 [A值时间范围] 注意：当前使用30天近似计算")
            print(f"🔍 [A值时间范围] 真实逻辑应该按套餐周期计算（开通日期+N个月）")
            
            print(f"🔍 [A值API] 使用POST方法调用聚合使用费用API")
            print(f"🔍 [A值API] 请求参数: teamId=-1, 时间范围: 近似30天（应为套餐周期）")
            ...
            if response.status_code == 200:
                ...
                if aggregations:
                    for agg in aggregations:
                        ...
                        print(f"🔍 [模型使用] {model}: 输入{input_tokens}, 输出{output_tokens}, 缓存读取{cache_read}, 缓存写入{cache_write}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 500:
                    print(f"🚨 月度账单API服务器错误(500) - 这是Cursor服务器问题，不是代码问题")
    ...
    def _retry_monthly_invoice_with_events(self, session_token, month, year):
        ...
        try:
            ...
            request_body = {
                "month": month,
                "year": year,
                "includeUsageEvents": True  # 包含使用事件
            }
