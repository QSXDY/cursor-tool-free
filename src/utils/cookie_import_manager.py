        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                try:
                    # 🔧 解析每行 - 支持多种格式
                    if '%3A%3A' in line:
                        success, message, account_info = self._parse_complete_format(line, skip_subscription=skip_subscription)
                    elif line.startswith('ey'):
                        success, message, account_info = self._parse_jwt_only_format(line, skip_subscription=skip_subscription)
                    else:
