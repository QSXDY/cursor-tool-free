...
class SubscriptionBatchProcessor:
    ...
    def process_accounts_batch(self, accounts_list: List[Dict], preserve_remarks: bool = False, progress_callback: Optional[Callable] = None) -> Tuple[bool, str, List[Dict]]:
        """
        批量处理账号订阅信息 - 
    ...
    def _fetch_subscriptions_concurrently(self, accounts_list: List[Dict], preserve_remarks: bool = False, progress_callback: Optional[Callable] = None) -> List[Dict]:
        """并发获取订阅状态 - """
        total_accounts = len(accounts_list)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_index.keys()):
                try:
                    ...
                    if progress_callback:
                        progress_callback(completed_count, total_accounts, f'已处理 {completed_count}/{total_accounts} 个账号')
