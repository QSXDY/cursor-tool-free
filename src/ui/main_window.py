...
from ..utils.cookie_import_manager import CookieImportManager, CookieParser
from ..utils.cursor_version import CursorVersionDetector
...
class CursorAccountManagerPro(QMainWindow):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted and dialog.account_data:
            ...
            if success:
                QMessageBox.information(self, "导入成功", 
                    f"账号 {dialog.account_data.get('email', 'N/A')} 导入成功")
    ...
    def _apply_account_config(self, account, storage_machine_ids, machine_id_file_value):
        ...
        try:
            ...
            # 验证关键配置
            print("🔍 验证写入的认证配置:")
            print(f"   workos.sessionToken: {new_config['workos.sessionToken'][:50] if new_config['workos.sessionToken'] else 'N/A'}...")
    ...
    def delete_selected(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            remaining_accounts = [acc for acc in all_accounts 
                                if acc.get('user_id') not in user_ids_to_delete]
        ...
        for row in range(self.account_table.rowCount()):
            if row < len(all_accounts):
                ...
                if account.get('status') == '已应用':
                    applied_rows_and_accounts.append({
                        'row': row,
                        'account': account
                    })
    ...
    def update_usage_data(self):
        ...
        # 取消之前的更新线程
        if hasattr(self, '_current_update_thread') and self._current_update_thread and self._current_update_thread.isRunning():
    ...
    def handle_usage_update(self, account_details):
        ...
        # 使用富文本，只有订阅类型值变色
        self.current_subscription_label.setText(f'订阅类型: <span style="color: {color_hex};">{subscription_display}</span>')
    ...
    def update_usage_cost_display(self, aggregated_usage_cost, monthly_invoice_cost):
        ...
        try:
            ...
            display_text = f'({int(round(aggregated_usage_cost))}+{int(round(monthly_invoice_cost))}={int(round(total_sum))}$)'
            ...
            self.current_usage_label.setToolTip(f'{tooltip} - a值(聚合费用): {aggregated_usage_cost:.2f}$, b值(月度账单): {monthly_invoice_cost:.2f}$')
