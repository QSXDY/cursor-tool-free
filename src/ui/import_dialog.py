...
⚠️ 提示：仅支持JWT token，无需其他信息
    ...
    def _do_parse(self, cookie_text):
        ...
        try:
            ...
            success, message, accounts_list = self.cookie_manager.parse_batch_cookie_info(cookie_text, skip_subscription=True)
            ...
            if len(accounts_list) != 1:
                self._show_error(f"🔒 安全限制：一次只能导入一个账号\n\n"
            ...
            if account_info:
                ...
                print(f"✅ 单账号API解析成功，邮箱: {account_info.get('email')}, 订阅: {account_info.get('subscription_type')}")
            else:
                self._show_error(f"❌ 获取账号信息失败")
                self.account_data = None
